...
# modified distance function, see figure 2 in the paper
...
def k_completion(points, centers, radii, k):
    ...
    return centers  # skipping the assignment compared to the paper, instead implementing it later


# produces all possible interval borders within which a radius could be, NOT k radii
def guessing_radii(points, k):
    ...
    j = 1 # iterator variable/index

    # each profile is initialized with a sub interval of the largest possible solution, which would be one large radius aka the max k center radius multiplied with k
    possible_radii_profiles.append([pow(1 + epsilon, j - 1) * (max_radius / beta)])  # lower bound
    while (j < math.log(beta * k, 1 + epsilon) and possible_radii_profiles[-1] < max_radius * k):
    ...
    for profile in possible_radii_profiles:
        ...
        while (j <= math.log(k / epsilon, 1 + epsilon) and profile[-1] <= profile[0]):
            ...
            j += 1
        # put the largest radius of the interval at the end to have the profile sorted
...
# check if a solution actually solves the cover problem
...
# a solution like that can be viable, if less than k centers lead to a better result than any guesses with k centers
def get_assignment_tuples(k):
    return list(itertools.combinations_with_replacement(range(k), k))

# because the guessed interval lists contain more than k intervals, they need to be turned into lists of
# k radius values, that cover all possibilities produced by the intervals
...
def approximate(points, k):
    ...
    final_centers = []
    upper_bound = (2 + 0.4) * max([euclidean(a, b) for a in points for b in points])  # using a 2 + epsilon approximation the sum of all radii can't be larger than 2 + epsilon multiplied with the largest distance
    print("Guessing radii...")

    radius_profile_candidates = guessing_radii(points, k)  # getting intervals
